...
public class AdminViewMenu extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (req.getSession().getAttribute("admin") == null) {
			...
			// Logic to fetch data from database // "dao" is used to deal with database
			MyDao dao = new MyDao();
			List<FoodItem> items = dao.fetchAllFoodItem();

			/// <-------------NOT NEEDED JUST FOR INFORMATION----------->////

			//// THIS IS WRITING HTML AND JAVA CODE IN SAME WITHOUT USING JSP////
...
//		resp.getWriter().print(
//				"<tr><th>"+item.getName()+"</th><th>"+item.getType()+"</th><th></th><th>"+item.getQuantity()+"</th><th><button>Edit</button></th><th><button>Delete</button></th></tr>");
//	}
